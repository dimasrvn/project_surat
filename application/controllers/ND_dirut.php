...
class ND_dirut extends CI_Controller {
	...
	public function index(){
		$page = 'utama';
		$s = $this->mymodel->Select('nd_dirut', 'ORDER BY id_surat DESC');
		$data = array('data' => $s);
		...
		$this->load->view('ND_dirut/index', $comp);
	...
	public function insert(){
		...
		$this->load->view('ND_dirut/insert', $comp);
	}
	...
	public function do_insert(){
		...
		$no_surat = $this->input->post('no');
		$res = $this->mymodel->Insert('nd_dirut', $data);
		$s=$this->mymodel->Select('nd_dirut', 'where no_surat = "'.$no_surat.'"');
		foreach ($s as $row)
		{
		   $id = $row->id_surat;
		}
		...
		if($res >= 1){
			...
			redirect('ND_dirut/upload_gambar/'.$id.'');
		  //echo "insert sukses";
		}
	}
	...
	public function update($id){
		...
		$page = 'utama';
		$list = $this->mymodel->Select("nd_dirut", "WHERE id_surat = '$id' ");
		...
		$this->load->view('ND_dirut/update',$comp);
	}
	public function do_update($id){
		...
		$res = $this->mymodel->Update('nd_dirut', $data, $where);

		if($res >= 1){
			$this->session->set_flashdata('pesan', 'sukses_edit');
			redirect('ND_dirut/index');
		  //echo "insert sukses";
		}
	}
	...
	public function do_delete($id){
		...
		$res = $this->mymodel->Delete('nd_dirut', $data);
	...
	public function upload_gambar(){
		...
		$this->load->view('ND_dirut/upload_gambar', $comp);
	}
		...
		//mkdir($dir);
		  if($_FILES["files"]["name"] != '')
		  {
		   ...
		   $this->upload->initialize($config);
			 ...
			 $query = $this->db->query("SELECT * FROM berkas WHERE id_surat ='$id' AND jenis ='nd_dirut'");
			 $c = $query->num_rows();
		   ...
		   for($count = 0; $count<count($_FILES["files"]["name"]); $count++)
		   {
		   	$filename = $_FILES["files"]["name"][$count];
				...
				$newfilename = $id."-nd-direktur-utama-".$i . $file_ext;
	...
	public function lihat_gambar($id){
		...
		$s = $this->mymodel->Select("berkas", "WHERE id_surat = '$id' AND jenis ='ND_dirut'");
		$data = array('data' => $s);
		...
		$sesi = array('id'=>$id);
		$this->session->set_userdata($sesi);
		$this->load->view('ND_dirut/lihat_gambar', $comp);
	}
	...
	public function do_delete_gambar($id_surat,$id){
		...
		if($res >= 1){
			$this->session->set_flashdata('pesan', 'sukses_hapus');
			redirect('ND_dirut/lihat_gambar/'.$id_surat.'');
		}
	}
    ...
    $exists = $this->User_model->filename_exists('nd_dirut', $no);
