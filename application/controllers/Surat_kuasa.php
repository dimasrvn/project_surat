...
class Surat_kuasa extends CI_Controller {
	...
	public function index(){
		$page = 'kuasa';
		$s = $this->mymodel->Select('s_kuasa', 'ORDER BY id_surat DESC');
		$data = array('data' => $s);
		...
		$this->load->view('surat_kuasa/index', $comp);
	...
	public function insert(){
		...
		$this->load->view('surat_kuasa/insert', $comp);
	}
	...
	public function do_insert(){
		...
		$no_surat = $this->input->post('no');
		$res = $this->mymodel->Insert('s_kuasa', $data);
		$s=$this->mymodel->Select('s_kuasa', 'where no_surat = "'.$no_surat.'"');
		foreach ($s as $row)
		{
		   $id = $row->id_surat;
		}
		...
		if($res >= 1){
			...
			redirect('surat_kuasa/upload_gambar/'.$id.'');
		  //echo "insert sukses";
		}
	}
	...
	public function update($id){
		...
		$page = 'kuasa';
		$list = $this->mymodel->Select("s_kuasa", "WHERE id_surat = '$id' ");
		...
		$this->load->view('surat_kuasa/update',$comp);
	}
	public function do_update($id){
		...
		$res = $this->mymodel->Update('s_kuasa', $data, $where);

		if($res >= 1){
			$this->session->set_flashdata('pesan', 'sukses_edit');
			redirect('surat_kuasa/index');
		  //echo "insert sukses";
		}
	}
	...
	public function do_delete($id){
		...
		$res = $this->mymodel->Delete('s_kuasa', $data);
	...
	public function upload_gambar(){
		...
		$this->load->view('surat_kuasa/upload_gambar', $comp);
	}
		...
		//mkdir($dir);
		  if($_FILES["files"]["name"] != '')
		  {
		   ...
		   $this->upload->initialize($config);
			 ...
			 $query = $this->db->query("SELECT * FROM berkas WHERE id_surat ='$id' AND jenis='kuasa' ");
			 $c = $query->num_rows();
		   ...
		   for($count = 0; $count<count($_FILES["files"]["name"]); $count++)
		   {
		   	$filename = $_FILES["files"]["name"][$count];
				...
				$newfilename = $id."-surat-kuasa-". $i . $file_ext;
	...
	public function lihat_gambar($id){
		...
		$s = $this->mymodel->Select("berkas", "WHERE id_surat = '$id' AND jenis='kuasa' ");
		$data = array('data' => $s);
		...
		$sesi = array('id'=>$id);
		$this->session->set_userdata($sesi);
		$this->load->view('surat_kuasa/lihat_gambar', $comp);
	}
    ...
    $exists = $this->User_model->filename_exists('s_kuasa', $no);
