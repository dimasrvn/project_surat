...
class ND_dirkeu extends CI_Controller {
	...
	public function index(){
		$page = 'keuangan';
		$s = $this->mymodel->Select('ND_dirkeu', 'ORDER BY id_surat DESC');
		$data = array('data' => $s);
		...
		$this->load->view('ND_dirkeu/index', $comp);
	...
	public function insert(){
		...
		$this->load->view('ND_dirkeu/insert', $comp);
	}
	...
	public function do_insert(){
		...
		$no_surat = $this->input->post('no');
		$res = $this->mymodel->Insert('ND_dirkeu', $data);
		$s=$this->mymodel->Select('ND_dirkeu', 'where no_surat = "'.$no_surat.'"');
		foreach ($s as $row)
		{
		   $id = $row->id_surat;
		}
		...
		if($res >= 1){
			...
			redirect('ND_dirkeu/upload_gambar/'.$id.'');
		  //echo "insert sukses";
		}
	}
	...
	public function update($id){
		...
		$page = 'keuangan';
		$list = $this->mymodel->Select("ND_dirkeu", "WHERE id_surat = '$id' ");
		...
		$this->load->view('ND_dirkeu/update',$comp);
	}
	public function do_update($id){
		...
		$res = $this->mymodel->Update('ND_dirkeu', $data, $where);

		if($res >= 1){
			$this->session->set_flashdata('pesan', 'sukses_edit');
			redirect('ND_dirkeu/index');
		  //echo "insert sukses";
		}
	}
	...
	public function do_delete($id){
		...
		$res = $this->mymodel->Delete('ND_dirkeu', $data);
	...
	public function upload_gambar(){
		...
		$this->load->view('ND_dirkeu/upload_gambar', $comp);
	}
		...
		//mkdir($dir);
		  if($_FILES["files"]["name"] != '')
		  {
		   ...
		   $this->upload->initialize($config);
			 ...
			 $query = $this->db->query("SELECT * FROM berkas WHERE id_surat ='$id' AND jenis ='nd_dirkeu'");
			 $c = $query->num_rows();
		   ...
		   for($count = 0; $count<count($_FILES["files"]["name"]); $count++)
		   {
		   	$filename = $_FILES["files"]["name"][$count];
				...
				$newfilename = $id."-nd-direktur-keuangan-".$i . $file_ext;
	...
	public function lihat_gambar($id){
		...
		$s = $this->mymodel->Select("berkas", "WHERE id_surat = '$id' AND jenis ='nd_dirkeu'");
		$data = array('data' => $s);
		...
		$this->load->view('ND_dirkeu/lihat_gambar', $comp);
	}
	...
	public function do_delete_gambar($id_surat,$id){
		...
		if($res >= 1){
			$this->session->set_flashdata('pesan', 'sukses_hapus');
			redirect('ND_dirkeu/lihat_gambar/'.$id_surat.'');
		}
	}
    ...
    $exists = $this->User_model->filename_exists('ND_dirkeu', $no);
