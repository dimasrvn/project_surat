...
class Surat_kpts extends CI_Controller {
	...
	public function index(){
		$page = 'kpts';
		$s = $this->mymodel->Select('s_kpts', 'ORDER BY id_surat DESC');
		$data = array('data' => $s);
		...
		$this->load->view('surat_kpts/index', $comp);
	...
	public function insert(){
		...
		$this->load->view('surat_kpts/insert', $comp);
	}
	...
	public function do_insert(){
		...
		$no_surat = $this->input->post('no');
		$res = $this->mymodel->Insert('s_kpts', $data);
		$s=$this->mymodel->Select('s_kpts', 'where no_surat = "'.$no_surat.'"');
		foreach ($s as $row)
		{
		   $id = $row->id_surat;
		}
		...
		if($res >= 1){
			...
			redirect('surat_kpts/upload_gambar/'.$id.'');
		  //echo "insert sukses";
		}
	}
	...
	public function update($id){
		...
		$page = 'kpts';
		$list = $this->mymodel->Select("s_kpts", "WHERE id_surat = '$id' ");
		...
		$this->load->view('surat_kpts/update',$comp);
	}
	public function do_update($id){
		...
		$res = $this->mymodel->Update('s_kpts', $data, $where);

		if($res >= 1){
			$this->session->set_flashdata('pesan', 'sukses_edit');
			redirect('surat_kpts/index');
		  //echo "insert sukses";
		}
	}
	...
	public function do_delete($id){
		...
		$res = $this->mymodel->Delete('s_kpts', $data);
	...
	public function upload_gambar(){
		...
		$this->load->view('surat_kpts/upload_gambar', $comp);
	}
		...
		//mkdir($dir);
		  if($_FILES["files"]["name"] != '')
		  {
		   ...
		   $this->upload->initialize($config);
			 ...
			 $query = $this->db->query("SELECT * FROM berkas WHERE id_surat ='$id' AND jenis='kpts' ");
			 $c = $query->num_rows();
		   ...
		   for($count = 0; $count<count($_FILES["files"]["name"]); $count++)
		   {
		   	$filename = $_FILES["files"]["name"][$count];
				...
				$newfilename = $id."-surat-kpts-". $i . $file_ext;
	...
	public function lihat_gambar($id){
		...
		$s = $this->mymodel->Select("berkas", "WHERE id_surat = '$id' AND jenis='kpts' ");
		$data = array('data' => $s);
		...
		$sesi = array('id'=>$id);
		$this->session->set_userdata($sesi);
		$this->load->view('surat_kpts/lihat_gambar', $comp);
	}
	...
	public function do_delete_gambar($id_surat,$id){
		...
		if($res >= 1){
			$this->session->set_flashdata('pesan', 'sukses_hapus');
			redirect('surat_kpts/lihat_gambar/'.$id_surat.'');
		}
	}
    ...
    $exists = $this->User_model->filename_exists('s_kpts', $no);
