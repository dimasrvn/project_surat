...
class MOU extends CI_Controller {
	...
	public function index(){
		$page = 'mou';
		$s = $this->mymodel->Select('mou', 'ORDER BY id_surat DESC');
		$data = array('data' => $s);
	...
	public function insert(){
		...
		$this->load->view('mou/insert', $comp);
	}
	...
	public function do_insert(){
		...
		$no_surat = $this->input->post('no');
		$res = $this->mymodel->Insert('mou', $data);
		$s=$this->mymodel->Select('mou', 'where no_surat = "'.$no_surat.'"');
		foreach ($s as $row)
		{
		   $id = $row->id_surat;
		}
	...
	public function update($id){
		...
		$page = 'mou';
		$list = $this->mymodel->Select("mou", "WHERE id_surat = '$id' ");
		...
		$this->load->view('mou/update',$comp);
	}
	public function do_update($id){
		...
		$res = $this->mymodel->Update('mou', $data, $where);
	...
	public function do_delete($id){
		...
		$res = $this->mymodel->Delete('mou', $data);
	...
	public function upload_gambar(){
		...
		$this->load->view('mou/upload_gambar', $comp);
	}
		...
		//mkdir($dir);
		  if($_FILES["files"]["name"] != '')
		  {
		   ...
		   $this->upload->initialize($config);
			 ...
			 $query = $this->db->query("SELECT * FROM berkas WHERE id_surat ='$id' AND jenis='mou' ");
			 $c = $query->num_rows();
	...
	public function lihat_gambar($id){
		...
		$s = $this->mymodel->Select("berkas", "WHERE id_surat = '$id' AND jenis='mou' ");
		$data = array('data' => $s);
		...
		$sesi = array('id'=>$id);
		$this->session->set_userdata($sesi);
		$this->load->view('mou/lihat_gambar', $comp);
	}
	...
	public function do_delete_gambar($id_surat,$id){
		...
		if($res >= 1){
			$this->session->set_flashdata('pesan', 'sukses_hapus');
			redirect('mou/lihat_gambar/'.$id_surat.'');
		}
	}
    ...
    $exists = $this->User_model->filename_exists('mou', $no);
